...
func formatToolDiscoveryRule(useBM25, useRegex bool, phase GoalPhase) string {
	...
	// Phase 12.48b site 10: policy-driven lookup. Phases with
	// DiscoveryRuleText set return the pinned rule; Open + empty fall
	// through to the legacy "MUST search" branch (preserves Phase 12.5
	// / 12.18.1 behavior for Open).
	if policy := PhasePolicyFor(phase); policy != nil && policy.DiscoveryRuleText != "" {
		return policy.DiscoveryRuleText
	}
