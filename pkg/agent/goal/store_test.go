...
// TestStore_UpdateStatusSnapshot_Success verifies Phase 7 (plan §3.7)
// store.UpdateStatusSnapshot workflow: it writes a snapshot into the active
// goal's StatusSnapshot field and reads it back consistently.
func TestStore_UpdateStatusSnapshot_Success(t *testing.T) {
	store, workspace := newTestStore(t)
	_ = workspace

	g := Goal{Name: "test-goal", Description: Description{Objective: "test", SuccessCriteria: []string{"done"}}, Status: StatusActive, CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
	if err := store.Write("sess-snap", &g); err != nil {
		t.Fatalf("Write: %v", err)
	}
	loaded, err := store.LoadStatusSnapshot("sess-snap")
	if err != nil {
		t.Fatalf("LoadStatusSnapshot: %v", err)
	}
	if loaded != "" {
		t.Fatalf("initial snapshot = %q, want empty", loaded)
	}
	if err := store.UpdateStatusSnapshot("sess-snap", "Phase 7 in progress"); err != nil {
	...
	if loaded != "Phase 7 in progress" {
		t.Fatalf("after update snapshot = %q, want %q", loaded, "Phase 7 in progress")
	}
	g2, err := store.Read("sess-snap")
	if err != nil {
		t.Fatalf("Read: %v", err)
	}
	if g2.Status != g.Status {
		t.Fatalf("status changed: %q vs %q", g2.Status, g.Status)
	}
	if g2.Name != g.Name {
		t.Fatalf("name changed: %q vs %q", g2.Name, g.Name)
	}
}

func TestStore_UpdateStatusSnapshot_NoActiveGoal(t *testing.T) {
	store, workspace := newTestStore(t)
	_ = workspace
	err := store.UpdateStatusSnapshot("sess-none", "anything")
	if err == nil {
		t.Fatalf("UpdateStatusSnapshot on missing goal: err = nil, want ErrNoActiveGoal")
	}
	if !errors.Is(err, ErrNoActiveGoal) {
		t.Fatalf("err = %v, want ErrNoActiveGoal", err)
	}
	loaded, loadErr := store.LoadStatusSnapshot("sess-none")
	if loadErr != nil {
		t.Fatalf("LoadStatusSnapshot: %v", loadErr)
	}
	if loaded != "" {
		t.Fatalf("LoadStatusSnapshot on missing goal = %q, want empty", loaded)
	}
}

func TestStore_UpdateStatusSnapshot_Idempotent(t *testing.T) {
	store, workspace := newTestStore(t)
	_ = workspace

	g := Goal{Name: "test-goal", Description: Description{Objective: "test", SuccessCriteria: []string{"done"}}, Status: StatusActive, CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
	if err := store.Write("sess-idem", &g); err != nil {
		t.Fatalf("Write: %v", err)
	}
	g1, _ := store.Read("sess-idem")
	updated1 := g1.UpdatedAt

	if err := store.UpdateStatusSnapshot("sess-idem", "snapshot-A"); err != nil {
		t.Fatalf("first update: %v", err)
	}
	g2, _ := store.Read("sess-idem")
	updated2 := g2.UpdatedAt

	if err := store.UpdateStatusSnapshot("sess-idem", "snapshot-A"); err != nil {
		t.Fatalf("second update (same value): %v", err)
	}
	g3, _ := store.Read("sess-idem")
	updated3 := g3.UpdatedAt

	if !updated2.After(updated1) {
		t.Fatalf("first update must bump UpdatedAt: %v vs %v", updated1, updated2)
	}
	if !updated3.Equal(updated2) {
		t.Fatalf("second update (same value) must NOT bump UpdatedAt: %v vs %v", updated2, updated3)
	}
}

func TestStore_UpdateStatusSnapshot_EmptyKey_Rejected(t *testing.T) {
	store, workspace := newTestStore(t)
	_ = workspace
	if err := store.UpdateStatusSnapshot("", "snap"); err == nil {
		t.Fatal("empty session key must error")
	}
}

func TestStore_LoadStatusSnapshot_CrossSession(t *testing.T) {
	store, workspace := newTestStore(t)
	_ = workspace

	g := Goal{Name: "test-goal", Description: Description{Objective: "test", SuccessCriteria: []string{"done"}}, Status: StatusActive, CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
	if err := store.Write("sess-A", &g); err != nil {
		t.Fatalf("Write A: %v", err)
	}
	if err := store.UpdateStatusSnapshot("sess-A", "alpha"); err != nil {
		t.Fatalf("Update A: %v", err)
	}

	loaded, err := store.LoadStatusSnapshot("sess-B")
	if err != nil {
		t.Fatalf("LoadStatusSnapshot B: %v", err)
	}
	if loaded != "" {
		t.Fatalf("sess-B leaked from sess-A: %q", loaded)
	}
}
